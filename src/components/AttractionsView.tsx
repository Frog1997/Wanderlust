...
export const AttractionsView: React.FC<AttractionsViewProps> = ({
  ...
  const handleSaveAttraction = (e: React.FormEvent) => {
    ...
    const newAtt: AttractionBookmark = {
      id: `att-${Date.now()}`,
      title: newTitle.trim(),
    ...
    onUpdateTrip({
      ...trip,
      attractions: [newAtt, ...trip.attractions],
    });
  ...
  return (
    <div className="space-y-6">
      {/* Toast Notification */}
      {toastMessage && (
        <div className="fixed top-20 right-6 z-50 bg-stone-900 text-white px-4 py-3 rounded-2xl shadow-xl border border-stone-700 flex items-center gap-2 animate-in fade-in slide-in-from-top-2 text-xs font-bold">
      ...
      <div className="bg-white dark:bg-neutral-900 p-5 rounded-2xl border border-stone-200 dark:border-neutral-700 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
        ...
        <div className="flex items-center gap-3">
          <div className="relative flex-1 md:w-64">
            <Search className="w-4 h-4 text-stone-400 dark:text-neutral-600 absolute left-3 top-1/2 -transtone-y-1/2" />
            <input
              type="text"
              placeholder="搜尋景點、標籤或餐廳..."
              value={searchQuery}
              onChange={(e) => setSearchQuery(e.target.value)}
              className="w-full pl-9 pr-3.5 py-2 bg-stone-50 dark:bg-neutral-950 rounded-xl border border-stone-200 dark:border-neutral-700 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500"
            />
          </div>
          ...
          <button
            ...
            className={`px-3.5 py-2 rounded-xl text-xs font-bold whitespace-nowrap transition-all flex items-center gap-1.5 ${
              selectedCategory === tab.id
                ...
                : 'bg-white dark:bg-neutral-900 text-stone-600 dark:text-neutral-400 border border-stone-200 dark:border-neutral-700 hover:bg-stone-50 dark:hover:bg-neutral-800 dark:bg-neutral-950'
            }`}
          >
            <span>{tab.label}</span>
            <span className={`text-[10px] px-1.5 py-0.5 rounded-md ${
              selectedCategory === tab.id ? 'bg-white/20 dark:bg-neutral-900/20 text-white' : 'bg-stone-100 dark:bg-neutral-800 text-stone-500 dark:text-neutral-500'
            }`}>
              {tab.count}
            </span>
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
        {filteredAttractions.map((att) => (
          <div
            key={att.id}
            className="bg-white dark:bg-neutral-900 rounded-2xl border border-stone-200 dark:border-neutral-700 shadow-sm overflow-hidden flex flex-col hover:border-stone-300 dark:border-neutral-600 hover:shadow-md transition-all group"
          >
            {/* Cover Image with badges */}
            <div className="relative h-44 w-full bg-stone-100 dark:bg-neutral-800 overflow-hidden">
              <img
                src={att.imageUrl}
                alt={att.title}
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300"
              />
              <div className="absolute inset-0 bg-gradient-to-t from-stone-900/70 via-transparent to-black/20" />
            ...
            <div className="p-4 flex-1 flex flex-col justify-between space-y-3">
              <div>
                <div className="flex items-start justify-between gap-2">
                  <h3 className="text-base font-bold text-stone-900 dark:text-white line-clamp-1">{att.title}</h3>
                  {att.estimatedCost && (
                ...
                <div className="flex items-center gap-1 text-xs text-stone-500 dark:text-neutral-500 mt-1">
                  <MapPin className="w-3.5 h-3.5 text-stone-400 dark:text-neutral-600 flex-shrink-0" />
                  <span className="truncate">{att.address}</span>
                </div>

                {att.openingHours && (
                  <div className="flex items-center gap-1 text-xs text-stone-500 dark:text-neutral-500 mt-1">
                    <Clock className="w-3.5 h-3.5 text-stone-400 dark:text-neutral-600 flex-shrink-0" />
                    <span>{att.openingHours}</span>
                  </div>
                )}

                {att.notes && (
                  <p className="text-xs text-stone-600 dark:text-neutral-400 bg-stone-50 dark:bg-neutral-950 p-2.5 rounded-xl mt-2.5 border border-stone-100 dark:border-neutral-800 line-clamp-2 leading-relaxed">
                    {att.notes}
                  </p>
                )}
                ...
                <div className="flex flex-wrap gap-1.5 mt-3">
                  {att.tags.map((tag) => (
                    <span
                      key={tag}
                      className="px-2 py-0.5 bg-stone-100 dark:bg-neutral-800 text-stone-600 dark:text-neutral-400 text-[11px] font-medium rounded-md"
                    >
                      #{tag}
                    </span>
                  ))}
              ...
              {/* Bottom Actions */}
              <div className="pt-3 border-t border-stone-100 dark:border-neutral-800 flex items-center justify-between gap-2">
                <button
                ...
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="p-2 text-stone-400 dark:text-neutral-600 hover:text-primary-600 hover:bg-stone-50 dark:hover:bg-neutral-800 dark:bg-neutral-950 rounded-xl transition-all"
                  title="在 Google 地圖查看"
                >
                ...
                <button
                  ...
                  className="p-2 text-stone-300 hover:text-rose-600 hover:bg-rose-50 rounded-xl transition-all"
                  title="刪除"
                >
      ...
      {addingToDayModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-neutral-900 w-full max-w-md rounded-2xl shadow-2xl border border-stone-200 dark:border-neutral-700 p-5 space-y-4">
            <div className="flex justify-between items-center pb-2 border-b border-stone-100 dark:border-neutral-800">
              <h4 className="font-bold text-stone-900 dark:text-white text-base">排入指定天數行程</h4>
              <button
            ...
            <div>
              <span className="text-xs text-stone-500 dark:text-neutral-500">選定景點：</span>
              <div className="font-bold text-stone-900 dark:text-white text-sm mt-0.5">{addingToDayModal.title}</div>
            </div>

            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">選擇天數</label>
                <select
                  value={targetDayNumber}
                  onChange={(e) => setTargetDayNumber(Number(e.target.value))}
                  className="w-full px-3 py-2 rounded-xl border border-stone-200 dark:border-neutral-700 text-xs bg-white dark:bg-neutral-900 focus:outline-none focus:ring-2 focus:ring-primary-500"
                >
                  {trip.days.map((d) => (
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">預計抵達時間</label>
                <input
                  type="time"
                  value={targetTime}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-stone-900/60 backdrop-blur-sm animate-in fade-in">
          <div className="bg-white dark:bg-neutral-900 w-full max-w-lg rounded-2xl shadow-2xl border border-stone-200 dark:border-neutral-700 overflow-hidden">
            <div className="p-5 bg-stone-50 dark:bg-neutral-950 border-b border-stone-200 dark:border-neutral-700 flex justify-between items-center">
              <h4 className="font-bold text-stone-900 dark:text-white text-base">收藏新景點與美食</h4>
              <button
            ...
            <form onSubmit={handleSaveAttraction} className="p-5 space-y-4 max-h-[80vh] overflow-y-auto">
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">景點或名店名稱 *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">分類</label>
                  <select
                    value={newCategory}
                ...
                <div>
                  <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">評分 (1 ~ 5)</label>
                  <input
                    type="number"
                    step="0.1"
                    min="1"
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">地址 / 地點</label>
                <input
                  type="text"
                  placeholder="例如：東京都港区六本木6-10-1"
                  value={newAddress}
                  onChange={(e) => setNewAddress(e.target.value)}
                  className="w-full px-3.5 py-2 rounded-xl border border-stone-200 dark:border-neutral-700 text-xs focus:outline-none focus:ring-2 focus:ring-primary-500"
                />
              </div>

              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">營業時間</label>
                  <input
                    type="text"
                ...
                <div>
                  <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">預估花費 ({trip.targetCurrency})</label>
                  <input
                    type="number"
                    placeholder="0"
                    value={newCost || ''}
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">標籤 (用逗號隔開)</label>
                <input
                  type="text"
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">封面圖片網址 (URL)</label>
                <input
                  type="url"
              ...
              <div>
                <label className="block text-xs font-bold text-stone-700 dark:text-neutral-300 mb-1">特色筆記與心得</label>
                <textarea
                  rows={2}
